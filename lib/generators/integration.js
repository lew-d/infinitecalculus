...
function generateDifferential() {
    ...
    */
    var bases = [
        "A/A", "A/B", "B",
        "B+log(A)", "B+log(A)", "sin(A)", "cos(A)", "log(A)"
    ]
...
export default function Differentiation() {
    ...
    if (derivative(eq, 'x').toString() == "0") {
        return Differentiation()
    }
    var question = eq.toTex().replace("log", "ln").replace(/(\+|\-)[0-9]/g, "") + "+ C"

    var answer = "\\int\\enspace" + simplify(derivative(eq, 'x')).toTex().replace("log", "ln") + "\\enspace dx"
